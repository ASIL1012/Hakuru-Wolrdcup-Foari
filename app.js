...
async function login(username) {
    ...
    const cloudData = await cloudLoadUserData(username);
    if (cloudData) {
        appData.accounts[username] = cloudData;
    } else {
        if (!appData.accounts[username]) {
            appData.accounts[username] = { predictions: {}, points: 6 };
        }
        // Save new player to cloud database so they are recorded on join!
        await cloudSavePoints(username, 6);
    }

    userState.username = username;
    userState.predictions = appData.accounts[username].predictions;
    userState.points = appData.accounts[username].points || 0;
...
function checkUser() {
    if (userState.username) {
        ...
        document.getElementById('profile-count').innerText = Object.keys(userState.predictions).length;
    } else {
...
function renderMatches() {
    ...
    upcomingMatches.forEach(match => {
        ...
        const isMatchOverrideLocked = (match.id === 10 || match.id === 11 || match.id === 12 || match.id === 13) && now.getTime() < new Date("2026-06-14T00:00:00Z").getTime();
        ...
        if (isPast) {
            ...
            lockStatusHTML = `<div class="lock-status locked">🔒 Locked until June 14</div>`;
    ...
    document.querySelectorAll('.match-card').forEach(card => {
        ...
        if (lockStatusEl) {
            ...
            if (isPast) {
                ...
                expectedText = `🔒 Locked until June 14`;
                expectedClass = 'lock-status locked';
            } else if (isToday || isTomorrow) {
