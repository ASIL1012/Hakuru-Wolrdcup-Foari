...
function renderMatches() {
    ...
    const cutoffTime = new Date("2026-07-04T17:00:00Z"); // Kickoff of Match 89 (Round of 16 start)
    ...
    if (!isSpecialEventLocked) {
        ...
        winnerCard.className = 'card match-card';
        ...
        if (savedTeamId) {
            const savedTeam = roundOf16Teams.find(t => t.id === savedTeamId);
            if (savedTeam) {
                savedPickHTML = `<span style="font-weight: 700; color: var(--secondary);">Your Champion Pick:</span> <img src="${savedTeam.flag}" class="team-flag" style="width: 20px; height: 14px; margin: 0 4px; vertical-align: middle; border-radius: 2px;"> ${savedTeam.name}`;
            }
        }
        ...
        matchContainer.appendChild(winnerCard);
...
function updateCountdowns() {
    ...
    document.querySelectorAll('.match-card').forEach(card => {
        ...
        if (matchId === 999) {
            const cutoffTime = new Date("2026-07-04T17:00:00Z").getTime();
