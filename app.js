...
function calculateScoreFromRawPredictions(rawPreds, boardType = 'overall') {
    ...
    if (boardType === 'overall' || boardType === 'group') {
        pts = 6;
        voted = 2;
    }
    ...
    rawPreds.forEach(p => {
        ...
        if (p.home_score !== null && p.away_score !== null && p.home_score !== '' && p.away_score !== '') {
            ...
            if (match && match.result) {
                const aHome = parseInt(match.result.home);
                const aAway = parseInt(match.result.away);

                const pHome = parseInt(p.home_score);
                const pAway = parseInt(p.away_score);

                let pOutcome = 'draw';
                if (pHome > pAway) pOutcome = 'home';
                else if (pHome < pAway) pOutcome = 'away';

                let aOutcome = 'draw';
                if (aHome > aAway) aOutcome = 'home';
                else if (aHome < aAway) aOutcome = 'away';
